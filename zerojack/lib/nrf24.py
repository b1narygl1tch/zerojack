...
class NRF24:
    ...
    def setPayloadSize(self, payload_size):
        # ACK = -1, DYNAMIC = 0, MIN = 1, MAX = 32
        assert -1 <= payload_size <= self.MAX_PAYLOAD_SIZE
        #self.payload_size = min(max(size, 1), self.MAX_PAYLOAD_SIZE)
    ...
    def writeAckPayload(self, pipe, buf, buf_len):
        ...
        #max_payload_size = 32
        data_len = min(buf_len, self.MAX_PAYLOAD_SIZE)
    ...
    def enableLNA(self): # Added method. Not sure if it works.
    ...
    def reset(self):
        ...
        reset_values = {0: 0x08, 1: 0x3F, 2: 0x02, 3: 0x03, 4: 0x03, 5: 0x02, 6: 0x06,
                        ...
                        0x1c: 0, 0x1d: 0}
        for reg, value in list(reset_values.items()):

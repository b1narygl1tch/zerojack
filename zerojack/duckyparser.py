...
class DuckyParser(object):
    ...
    def __init__(self, attack_script, layout='us'):
        key_mapping = keymap.mapping[layout.lower()]
    ...
    def parse(self):
        ...
        for line in self.script:
            ...
            # arrow keys
                ...
                entries.append(entry)
            # Fx keys
            elif re.match('^F([1-9]|1[0-2])$', line):
                entry = self.blank_entry.copy()
                stripped_line = line.strip()
                entry['char'] = stripped_line
                entry['hid'], entry['mod'] = self.char_to_hid(stripped_line)
                entries.append(entry)
                ...
                raise UserWarning(f'duckyparser.py cannot process: {line}')
